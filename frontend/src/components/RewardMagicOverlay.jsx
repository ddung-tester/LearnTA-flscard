...
function RewardMagicOverlay({
  ...
  useGSAP(
    () => {
      ...
      const leftTarget = compact
        ? layDiemDichPortal(
            portalRefs.current.center?.getBoundingClientRect(),
            "center"
          )
        : layDiemDichPortal(
    ...
    { scope: rootRef, dependencies: [active, sequenceKey, videoReady, originRect, combo, compact] }
  );
  ...
  return (
    <div
      ref={rootRef}
      className={`reward-magic ${compact ? "reward-magic--compact" : ""} ${
        fadeOut ? "reward-magic--fade-out" : ""
      }`}
      aria-hidden="true"
    >
