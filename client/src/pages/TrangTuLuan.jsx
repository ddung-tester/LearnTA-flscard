...
function TrangTuLuan() {
  ...
  const { combo, phase: comboPhase, incrementCombo, resetCombo, resetAll } = useCombo();
  ...
  useEffect(() => {
    let ds = chiHocTuYeuThich ? locTuYeuThich(danhSachGoc) : danhSachGoc;
    ...
    resetAll();
  }, [boId, lanLam, chiHocTuYeuThich, batRandom]);
  ...
  useEffect(() => {
    if (!daKiemTra || daHoanThanh || hienReward || !ketQuaDung) return undefined;

    const timer = setTimeout(() => chuyenCauMem(), lanReward > 0 ? 1000 : 2000);
    return () => clearTimeout(timer);
  }, [daKiemTra, ketQuaDung, daHoanThanh, hienReward, lanReward]);

  useEffect(() => {
    if (!hienReward) return undefined;
    const timer = setTimeout(() => setHienReward(false), CAU_HINH_REWARD_QUIZ.duration);
    return () => clearTimeout(timer);
  }, [hienReward]);
  ...
  function kiemTraDapAn(event) {
    ...
    if ((daKiemTra && ketQuaDung) || hienReward || dangChuyenCau || danhSachThe.length === 0) return;
  ...
  function boQua() {
    if (dangChuyenCau || hienReward) return;
    setDaKiemTra(true);
    setKetQuaDung(false);
    setDaBoQua(true);
    setCauTraLoi("");
  ...
  function doiCheDoReward() { setBatReward(p => !p); }
  ...
  if (daHoanThanh) {
    return (
      <>
        <RewardTikTokEffect active={batReward && hienReward} lanKichHoat={lanReward} config={CAU_HINH_REWARD_QUIZ} progressEndpointRef={progressEndpointRef} onHideComplete={xuLyRewardDongXong} combo={combo} />
